...
module UltracartClient
  class CouponQuery
    ...
    attr_accessor :exclude_expired

    # Expiration date begin
    attr_accessor :expiration_dts_begin
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::CouponQuery` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::CouponQuery`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'exclude_expired')
        self.exclude_expired = attributes[:'exclude_expired']
      end

      if attributes.key?(:'expiration_dts_begin')
        self.expiration_dts_begin = attributes[:'expiration_dts_begin']
      end

      if attributes.key?(:'expiration_dts_end')
