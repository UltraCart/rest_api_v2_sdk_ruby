  ...
  class OrderCoupon
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::OrderCoupon` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::OrderCoupon`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'automatically_applied')
        self.automatically_applied = attributes[:'automatically_applied']
      end

      if attributes.key?(:'base_coupon_code')
      ...
      if attributes.key?(:'hdie_from_customer')
    ...
    # @param [Object] base_coupon_code Value to be assigned
