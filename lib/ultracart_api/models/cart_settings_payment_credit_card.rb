...
module UltracartClient
  class CartSettingsPaymentCreditCard
    ...
    attr_accessor :hosted_fields_shopping_cart_token
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::CartSettingsPaymentCreditCard` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::CartSettingsPaymentCreditCard`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'hosted_fields_shopping_cart_token')
        self.hosted_fields_shopping_cart_token = attributes[:'hosted_fields_shopping_cart_token']
      end
    end
