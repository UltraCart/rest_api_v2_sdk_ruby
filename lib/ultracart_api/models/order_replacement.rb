  ...
  class OrderReplacement
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::OrderReplacement` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::OrderReplacement`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'additional_merchant_notes_new_order')
        self.additional_merchant_notes_new_order = attributes[:'additional_merchant_notes_new_order']
      end

      if attributes.key?(:'additional_merchant_notes_original_order')
      ...
      if attributes.key?(:'immediate_charge')
      ...
      if attributes.key?(:'skip_payment')
