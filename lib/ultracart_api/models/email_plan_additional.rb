  ...
  class EmailPlanAdditional
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::EmailPlanAdditional` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::EmailPlanAdditional`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'can_downgrade')
        self.can_downgrade = attributes[:'can_downgrade']
      end

      if attributes.key?(:'can_upgrade')
        self.can_upgrade = attributes[:'can_upgrade']
      end
      ...
      if attributes.key?(:'cost_change_formatted')
        self.cost_change_formatted = attributes[:'cost_change_formatted']
      end

      if attributes.key?(:'cost_formatted')
