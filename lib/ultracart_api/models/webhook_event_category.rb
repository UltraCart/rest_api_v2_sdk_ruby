  ...
  class WebhookEventCategory
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::WebhookEventCategory` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::WebhookEventCategory`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'any_subscribed')
        self.any_subscribed = attributes[:'any_subscribed']
      end

      if attributes.key?(:'available_expansions')
    ...
    def ==(o)
      ...
      self.class == o.class &&
          any_subscribed == o.any_subscribed &&
    ...
    def hash
      [any_subscribed, available_expansions, event_category, events, subscribed].hash
    end
