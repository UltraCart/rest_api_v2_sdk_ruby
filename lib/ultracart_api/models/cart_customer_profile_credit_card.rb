...
module UltracartClient
  class CartCustomerProfileCreditCard
    ...
    attr_accessor :customer_profile_credit_card_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::CartCustomerProfileCreditCard` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::CartCustomerProfileCreditCard`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'customer_profile_credit_card_id')
    ...
    def valid?
      card_type_validator = EnumAttributeValidator.new('String', ["AMEX", "Diners Club", "Discover", "MasterCard", "JCB", "VISA"])
    ...
    def hash
      [card_expiration_month, card_expiration_year, card_number, card_type, customer_profile_credit_card_id, last_used_date].hash
    end
