...
module UltracartClient
  class EmailPostcardStat
    ...
    attr_accessor :return_to_sender_count

    # Count of return to sender, formatted
    attr_accessor :return_to_sender_count_formatted
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::EmailPostcardStat` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::EmailPostcardStat`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'return_to_sender_count')
        self.return_to_sender_count = attributes[:'return_to_sender_count']
      end

      if attributes.key?(:'return_to_sender_count_formatted')
