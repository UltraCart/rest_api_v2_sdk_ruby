  ...
  class IntegrationLogQueryRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::IntegrationLogQueryRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::IntegrationLogQueryRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'item_ipn_oid')
        self.item_ipn_oid = attributes[:'item_ipn_oid']
      end

      if attributes.key?(:'log_dts_begin')
        self.log_dts_begin = attributes[:'log_dts_begin']
      end

      if attributes.key?(:'log_dts_end')
      ...
      if attributes.key?(:'logger_id')
        self.logger_id = attributes[:'logger_id']
      end

      if attributes.key?(:'logger_name')
