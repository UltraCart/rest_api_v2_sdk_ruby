  ...
  class RegisterAffiliateClickRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UltracartClient::RegisterAffiliateClickRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UltracartClient::RegisterAffiliateClickRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'affid')
        self.affid = attributes[:'affid']
      end
      ...
      if attributes.key?(:'referrer_url')
        self.referrer_url = attributes[:'referrer_url']
      end

      if attributes.key?(:'subid')
        self.subid = attributes[:'subid']
      end
    ...
    def hash
      [affid, ip_address, landing_page_url, referrer_url, subid, user_agent].hash
    end
